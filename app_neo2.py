...
#      1. DeepSeek baseline response (with uploaded data context)
#      2. DeepSeek refines the prompt for GPT-Neo (with uploaded data context)
#      3. GPT-Neo (with LoRA and references) generates a specialized answer (with uploaded data context)
#      4. DeepSeek merges the baseline and specialized responses (with uploaded data context)
...
# NEW: Folder for references that have already been used for training
...
def get_data_preview(user_id, num_rows=20) -> str:
    """
    Retrieve a CSV preview (first num_rows rows) of the uploaded data for a user.
    """
...
def deepseek_baseline_pass(user_msg: str, user_id: str) -> str:
    """
    Pass 1: Use DeepSeek to get a baseline (internal) response. The prompt now includes a data preview.
    """
    data_preview = get_data_preview(user_id, num_rows=20)
    prompt = (
        "You are DeepSeek+Olama providing a baseline internal thought. "
        "Do NOT simply repeat the user's text verbatim. "
...
def deepseek_refine_prompt_for_gptneo(user_msg: str, user_id: str) -> str:
    """
    Pass 2: Let DeepSeek produce a refined GPT-Neo prompt based on the user's query.
    The prompt now includes a data preview of the uploaded data.
    """
    data_preview = get_data_preview(user_id, num_rows=20)
    prompt = (
        "You are DeepSeek+Olama. The user gave the following request:\n\n"
        f"'{user_msg}'\n\n"
        "Below is the uploaded data (first 20 rows) for context:\n"
        f"{data_preview}\n\n"
...
def deepseek_supervise_merge(baseline_resp: str, specialized_resp: str, user_msg: str, user_id: str) -> str:
    """
    Pass 4: Let DeepSeek merge the baseline and specialized GPT-Neo responses into a final answer.
    The prompt includes a data preview as additional context.
    """
    data_preview = get_data_preview(user_id, num_rows=20)
    merge_prompt = (
        ...
        f"[Baseline]\n{baseline_resp}\n\n"
        f"[Specialized]\n{specialized_resp}\n\n"
        "The uploaded data for context (first 20 rows):\n"
        f"{data_preview}\n\n"
        f"User request:\n'{user_msg}'\n\n"
...
def build_training_dataset(user_id) -> str:
    """
    Create a .jsonl dataset from references, user feedback, and uploaded data.
    This dataset is later used for quick in-memory LoRA fine-tuning for GPT-Neo.
    References are processed only once: after being added to the dataset, they are moved
    to the TRAINED_REFERENCES_FOLDER.
    """
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    # Process references from the REFERENCES_FOLDER
    ...
    for fname in reference_files:
        ...
        # After processing, move the reference file to the trained references folder.
    ...
    # Incorporate user-uploaded data and description (if available)
    data_info = udata.get("data_info", {})
...
def fine_tune_model(user_id) -> str:
    """
    Perform a quick in-memory LoRA fine-tune of GPT-Neo on the references,
    user feedback, and uploaded data. Each reference is only used once.
    """
    global g_neo_model, g_neo_tokenizer
...
def read_filtered_references(user_msg: str) -> str:
    ...
    lines = []
    # Look for references in both active and already trained folders.
    for folder in [REFERENCES_FOLDER, TRAINED_REFERENCES_FOLDER]:
...
def gptneo_reference_pass(refined_prompt: str, user_msg: str, user_id: str) -> str:
    """
    Pass 3: Use GPT-Neo (with references and uploaded data) to generate a specialized answer.
    We combine the filtered references, a preview of the uploaded data, and the DeepSeek-refined prompt.
    """
    # Existing references from files
    refs_text = read_filtered_references(user_msg)

    # Initialize an empty data section text
    data_section = ""
    # Retrieve user-specific data (if any)
    udata = user_data.get(user_id, {})
    data_info = udata.get("data_info", {})

    # If a DataFrame has been uploaded, include a description and preview in the prompt
    if "df" in data_info and isinstance(data_info["df"], pd.DataFrame):
        df = data_info["df"]
        description = data_info.get("description", "").strip()
        data_preview = df.head(20).to_csv(index=False)
        data_section = (
            "User uploaded data is available.\n"
            f"Data Description: {description}\n"
            "Data Preview (first 20 rows):\n"
            f"{data_preview}\n\n"
        )

    # Build the final prompt including references, the uploaded data section (if any), and the refined prompt
    final_prompt = (
        ...
        f"{refined_prompt}\n\n"
        "Use these references and data only if they add domain-specific insight."
    )
...
def multi_step_generation(user_msg: str, user_id: str) -> str:
    ...
    Master pipeline:
       ...
       3) Generate a specialized answer using GPT-Neo with references and uploaded data.
       4) Merge the baseline and specialized answers using DeepSeek supervision.
    """
    baseline_answer = deepseek_baseline_pass(user_msg, user_id)
    refined_prompt = deepseek_refine_prompt_for_gptneo(user_msg, user_id)
    specialized_answer = gptneo_reference_pass(refined_prompt, user_msg, user_id)
    final_answer = deepseek_supervise_merge(baseline_answer, specialized_answer, user_msg, user_id)
    return final_answer
...
def process_chat(user_msg: str, user_id: str) -> str:
    ...
    # Pass user_id to the generation pipeline so that it can consider any uploaded data
